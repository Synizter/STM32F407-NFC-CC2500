...
typedef enum mrfiReceiveState
{
  MRFI_RECEIVE_IDLE = 0,
  MRFI_RECEIVE_READ_FRAME_LENGTH = 1,
  MRFI_RECEIVE_READ_PACKET = 2,
  MRFI_RECEIVE_CHECK_PACKET = 3
}mrfiReceiveState_t;
...
#define MRFI_RADIO_MIN_VERSION      3
#define MRFI_SETTING_PATABLE0       0x8D
#else
...
#define MRFI_ENABLE_SYNC_PIN_INT()                  MRFI_GDO0_EnableInterrupt()
#define MRFI_DISABLE_SYNC_PIN_INT()                 MRFI_GDO0_DisableInterrupt()
#define MRFI_SYNC_PIN_INT_IS_ENABLED()              MRFI_GDO0_IsInterruptEnable()
#define MRFI_CLEAR_SYNC_PIN_INT_FLAG()              MRFI_GDO0_ClearInterruptFlag()
#define MRFI_SYNC_PIN_INT_FLAG_IS_SET()             MRFI_GDO0_GetInterruptFlag()
#define MRFI_CONFIG_SYNC_PIN_FALLING_EDGE_INT()     MRFI_GDO0_ConfigInterruptFallingEdge()
...
#define MRFI_RXMODE_FIFOTHR          (0x0F)
#define MRFI_TXMODE_FIFOTHR          (0x00)
static const uint8_t mrfiTxFIFOSize[] = {61,57,53,49,45,41,37,33,29,25,21,17,13,9,5,1};
static const uint8_t mrfiRxFIFOSize[] = {4,8,12,16,20,24,28,32,36,40,44,48,52,56,60,64};
...
static void Mrfi_GDO0_ConfigAsSyncSignal(void);
static void Mrfi_GDO0_ConfigAsPAPD(void);
...
static uint8_t mrfiGDO0State;
static mrfiPacket_t mrfiIncomingPacket;
static void (*mrfiReceiveCallback)(mrfiPacket_t*);
static uint8_t mrfiRndSeed;
static volatile uint8_t mrfiPacketComming = 0;
static volatile uint8_t mrfiTransmitActive = 0;
static mrfiSignal_t mrfiGDO0Signal;
...
void MRFI_Init(void)
{
   ...
   *   -----------------
   */
  mrfiReceiveCallback = 0;
  
  MRFI_GDO0_SignalInit(&mrfiGDO0Signal);
  
  MRFI_SetRxModeControl(Mrfi_RxModeOff, Mrfi_RxModeOn);
  
  /* initialize GPIO pins */
  MRFI_GDO0_ConfigAsInput();
  ...
  MRFI_SPI_WriteByte(SRES);
  MRFI_SPI_WaitTransferDone();
...
uint8_t MRFI_Transmit(mrfiPacket_t * pPacket, uint8_t txType)
{
   ...
   *   --------------------------------
   */
  if(txBufLen > mrfiTxFIFOSize[MRFI_TXMODE_FIFOTHR])
  {
    mrfiSpiWriteTxFifo(txBuf, mrfiTxFIFOSize[MRFI_TXMODE_FIFOTHR]);
    txBuf += mrfiTxFIFOSize[MRFI_TXMODE_FIFOTHR];
    txCount = mrfiTxFIFOSize[MRFI_TXMODE_FIFOTHR];
  }
  else
  {
    mrfiSpiWriteTxFifo(txBuf, txBufLen);
    txCount = txBufLen;
  }
  
  /* 
   * There is no need for SYNC signal in this logic. So we
   * can re-configure the GDO_0 output from the radio to be PA_PD signal
   * instead of the SYNC signal.
   */
  Mrfi_GDO0_ConfigAsPAPD();
  ...
  else
  {
    ...
    for (;;)
    {
      ...
      while(!(mrfiSpiReadReg(PKTSTATUS) & (MRFI_PKTSTATUS_CCA | MRFI_PKTSTATUS_CS)));
  ...
  while(txBufLen - txCount > 0 && returnValue != MRFI_TX_RESULT_FAILED)
  {
    /* Check Tx FIFO */
    txBytesVerify = mrfiSpiReadReg(TXBYTES);
    do
    {
    ...
    if(txBytes < (mrfiTxFIFOSize[MRFI_TXMODE_FIFOTHR] / 4))
    {
      if((txBufLen - txCount) > (mrfiTxFIFOSize[MRFI_TXMODE_FIFOTHR] / 4))
      {
        mrfiSpiWriteTxFifo(txBuf, mrfiTxFIFOSize[MRFI_TXMODE_FIFOTHR] - txBytes);
        
        txBuf += (mrfiTxFIFOSize[MRFI_TXMODE_FIFOTHR] - txBytes);
        txCount += (mrfiTxFIFOSize[MRFI_TXMODE_FIFOTHR] - txBytes);
      }
      else
      {
        mrfiSpiWriteTxFifo(txBuf, (txBufLen - txCount));
        
        txCount += (txBufLen - txCount);
      }
    }
    else if(MRFI_SYNC_PIN_IS_HIGH())
    {
      returnValue = MRFI_TX_RESULT_FAILED;
      break;
    }
  ...
  timeout = 0xFFFF;
  while(!MRFI_SYNC_PIN_IS_HIGH() && timeout--)
  {
  }
...
void MRFI_ServiceRun(void)
{
  static enum mrfiReceiveState ReceiveState = MRFI_RECEIVE_IDLE;
  ...
  if(mrfiRadioState == MRFI_RADIO_STATE_RX)
  {
    if(!mrfiTransmitActive)
    {
      switch(ReceiveState)
      {
      case MRFI_RECEIVE_IDLE:
        /* Wait sync pin interrupt. */
        if(MRFI_GDO0_CheckSignal(&mrfiGDO0Signal))
          ReceiveState = MRFI_RECEIVE_READ_FRAME_LENGTH;
        break;
      case MRFI_RECEIVE_READ_FRAME_LENGTH:
        /* Check Rx FIFO */
        ...
        if(rxBytes >= MRFI_LENGTH_FIELD_SIZE)
        {
          /* read the first byte from FIFO - the packet length */
          mrfiSpiReadRxFifo(&frameLen, MRFI_LENGTH_FIELD_SIZE);
          
          /* Set rx buffer */
          frameBuf = &(mrfiIncomingPacket.frame[MRFI_FRAME_BODY_OFS]);
          
          /* Init receive count */
          rxCount = 0;
          
          ReceiveState = MRFI_RECEIVE_READ_PACKET;
        }
        else if((!MRFI_SYNC_PIN_IS_HIGH()))
        {
          /*
           ...
           */
          BSP_EnterCriticalSection(&s);
          MRFI_STROBE_IDLE_AND_WAIT();
          mrfiSpiCmdStrobe( SFRX );
          mrfiSpiCmdStrobe( SRX );
          BSP_ExitCriticalSection(&s);
          
          frameLen = 0;
          frameBuf = 0;
          
          /* Back to idle state. */
          ReceiveState = MRFI_RECEIVE_IDLE;
        }
        break;
        
      case MRFI_RECEIVE_READ_PACKET:
        ...
        if((!MRFI_SYNC_PIN_IS_HIGH()))
        {
           ...
           *  rxCount.  In that case, the radio automatically removes the packet from
           *  the FIFO.
           */
          if (rxBytes == 0 && rxCount == 0)
          {
          ...
          {
            if (((rxBytes + rxCount) != (frameLen + MRFI_RX_METRICS_SIZE)) ||
            ...
            else
            {
              ...
              mrfiSpiReadRxFifo(frameBuf, frameLen - rxCount);
              
              /* get receive metrics from FIFO */
              mrfiSpiReadRxFifo(&(mrfiIncomingPacket.rxMetrics[0]), MRFI_RX_METRICS_SIZE);
              
              ReceiveState = MRFI_RECEIVE_CHECK_PACKET;
            }
          }
        }
        else if(frameLen > mrfiRxFIFOSize[MRFI_RXMODE_FIFOTHR])
        {
          rxBytesVerify = mrfiSpiReadReg( RXBYTES );
          do
          {
          ...
          if(rxBytes >= (mrfiRxFIFOSize[MRFI_RXMODE_FIFOTHR]*3/4))
          {
            /* get packet from FIFO */
            mrfiSpiReadRxFifo(frameBuf, (rxBytes - 1));
            rxCount += (rxBytes - 1);
            frameBuf += (rxBytes - 1);
          }
        }
      ...
      case MRFI_RECEIVE_CHECK_PACKET:
          ...
          {
            {
              ...
              if(mrfiReceiveCallback)
                mrfiReceiveCallback(&mrfiIncomingPacket);
            }
          }
        }
...
static void Mrfi_RxModeOn(void)
{
  ...
  /* Set Rx FIFO Threshold to maximum (64 Bytes) */
  mrfiSpiWriteReg(FIFOTHR, MRFI_RXMODE_FIFOTHR);
 ...
 **************************************************************************************************
 */
void MRFI_RxOn(void (*recv_callback)(mrfiPacket_t*))
{
  /* radio must be awake before we can move it to RX state */
  MRFI_ASSERT( mrfiRadioState != MRFI_RADIO_STATE_OFF );
  
  mrfiReceiveCallback = recv_callback;
...
static void Mrfi_RxModeOff(void)
{
  ...
  /* Set Rx FIFO Threshold to minimum (4 Bytes) */
  mrfiSpiWriteReg(FIFOTHR, MRFI_TXMODE_FIFOTHR);
...
void MRFI_GpioIsr(void)
{
  ...
  if (MRFI_SYNC_PIN_INT_IS_ENABLED() && MRFI_SYNC_PIN_INT_FLAG_IS_SET())
  {
    ...
    if(mrfiGDO0State == MRFI_GDO_SYNC && !mrfiTransmitActive)
      MRFI_GDO0_ReleaseSignal(&mrfiGDO0Signal);
  }
}

/**************************************************************************************************
 * @fn          Mrfi_GDO0_ConfigAsSyncSignal
 *
 * @brief       Config GDO0 pin to Sync pin
 *
 * @param       none
 *
 * @return      none.
...
static void Mrfi_GDO0_ConfigAsSyncSignal(void)
{
  mrfiGDO0State = MRFI_GDO_SYNC;
  mrfiSpiWriteReg(IOCFG0, MRFI_GDO_SYNC);
}

/**************************************************************************************************
 * @fn          Mrfi_GDO0_ConfigAsPAPD
 *
 * @brief       Config GDO0 pin to Power Amplifer Pull Down
 *
 * @param       none
 *
 * @return      none.
...
static void Mrfi_GDO0_ConfigAsPAPD(void)
{
  mrfiGDO0State = MRFI_GDO_PA_PD;
  mrfiSpiWriteReg(IOCFG0, MRFI_GDO_PA_PD);
}
