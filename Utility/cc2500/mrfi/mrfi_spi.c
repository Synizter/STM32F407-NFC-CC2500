...
uint8_t mrfiSpiCmdStrobe(uint8_t addr)
{
  ...
  MRFI_SPI_ASSERT( MRFI_SPI_IsInit() );       /* SPI is not initialized */
  MRFI_SPI_ASSERT((addr >= 0x30) && (addr <= 0x3D));  /* invalid address */

  /* Take mutex that use SPI */
  MRFI_SPI_AcquireMutex();
  ...
  MRFI_SPI_WriteByte(addr);
  MRFI_SPI_WaitTransferDone();

  /* read the readio status byte returned by the command strobe */
  statusByte = MRFI_SPI_ReadByte();

  /* turn off chip select; Give mutex that call SPI functions */
  MRFI_SPI_TURN_CHIP_SELECT_OFF();
  
  MRFI_SPI_ReleaseMutex();
...
uint8_t mrfiSpiReadReg(uint8_t addr)
{
   ...
   */
  return( spiRegAccess(addr | BURST_BIT | READ__BIT, DUMMY_BYTE) );
}
...
{
  spiBurstFifoAccess(RXFIFO | BURST_BIT | READ__BIT, pData, len);
}
...
static void spiBurstFifoAccess(uint8_t addrByte, uint8_t * pData, uint8_t len)
{
  ...
  do
  {
    ...
    do
    {
       ...
       */
      if (addrByte & READ__BIT)
      {
        *pData = MRFI_SPI_ReadByte();
      }
